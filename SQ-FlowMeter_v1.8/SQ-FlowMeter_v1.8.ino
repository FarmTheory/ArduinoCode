...
float avgFlowRate[] = {};
int epromAddress[] = {0,16,32,48,64,70,76,82};
...
void loop() {
  ...
  if (wwOn == false && rateOn == false && calOn == false && historyOn == false){
      ...
      if (millis() - btUpdateTime >= 4000){
        char btBuffer[20]="";
        char bttemp[5]="";
        strcat(btBuffer,"FlowRate:");
        sprintf(bttemp,"%.1f",flowAvg.getAverage());
        strcat(btBuffer,bttemp);
        sendBluetooth(btBuffer);
        btUpdateTime = millis();
        }
...
void loadEEPROM(){
   ...
   if (eepromWrite == 1 || resetBtnState == 1){
      ...
      writeIntIntoEEPROM(epromAddress[2], 192);  //A1
      writeIntIntoEEPROM(epromAddress[3], 0); //V1
      writeIntIntoEEPROM(epromAddress[4], 932); //A2
      writeIntIntoEEPROM(epromAddress[5], 280); //V2
      writeIntIntoEEPROM(epromAddress[6], 1); //areaMetric  
   ...
   ww = readIntFromEEPROM(epromAddress[0])/10.0;
   tempWW = ww;
