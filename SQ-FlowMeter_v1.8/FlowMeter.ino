        ...
        if (i % 25 == 0) {
          ...
          if(areaMetric == true){
              switch (i / 25) {
                ...
                case -1: fmSprite.drawCentreString("70", flowBX+x0, flowBY+y0 - 9, 2); break;
                case 0: fmSprite.drawCentreString("140", flowBX+x0, flowBY+y0 - 7, 2); break;
                case 1: fmSprite.drawCentreString("210", flowBX+x0, flowBY+y0 - 9, 2); break;
                case 2: fmSprite.drawCentreString("280", flowBX+x0, flowBY+y0 - 12, 2); break;
              }
              }
          else{
            switch (i / 25) {
                case -2: fmSprite.drawCentreString("0", flowBX+x0, flowBY+y0 - 12, 2); break;
                case -1: fmSprite.drawCentreString("15k", flowBX+x0, flowBY+y0 - 9, 2); break;
                case 0: fmSprite.drawCentreString("31k", flowBX+x0, flowBY+y0 - 7, 2); break;
                case 1: fmSprite.drawCentreString("46k", flowBX+x0, flowBY+y0 - 9, 2); break;
                case 2: fmSprite.drawCentreString("62k", flowBX+x0, flowBY+y0 - 12, 2); break;
              }
            }
        }
        fmSprite.unloadFont();
...
int getFlow(){
    flowAnalog = analogAvg.getAverage();
...
void drawFlowMeter(int value, byte ms_delay)
{
  ...
  while (!(value == old_analog)) {
    ...
    float sdeg = map(old_analog, -24, 304, -150, -30); // Map value to angle
   ...
   // fmSprite.unloadFont(); 
    ...
    //CHANGE IF > 2m3 DIFF
    float diff = abs(value - prevFlowRate);
    if(diff < 2){
        value = prevFlowRate;
        }
    else{
