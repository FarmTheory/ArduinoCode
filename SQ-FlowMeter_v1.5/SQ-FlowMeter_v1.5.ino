...
void loop() {
  //MAIN SCREEN
  if (wwOn == false && rateOn == false && calOn == false){
      if (millis() - updateTime >= 1000) {
        ...
        drawCounter(applied+tempApplied, "applied (m3)");
        updateTime = millis();
        if (modeChanged == true){
           // Serial.println("MODE");
      ...
      if (millis() - updateTarget >= 10000){
        ...
        tempApplied = 0;
        drawCounter(applied, "applied (m3)");
        calcTarget();
        drawTarget(targetSpeed);
        updateTarget = millis();
        if (avgFlowRate > 0){
