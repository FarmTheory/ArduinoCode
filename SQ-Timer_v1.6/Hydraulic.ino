...
{ //hydTrigger
  ...
  tmSprite.setTextColor(GREY,c2);
  tmSprite.drawString("Set Hyd Trigger",timerWidth*0.5, timerHeight*0.2);
  char hydBuffer[20]="";
  char hydtemp[5]="";
//  Serial.println(value);
  sprintf(hydtemp,"%03d",hydTrigger);
  strcat(hydBuffer,hydtemp);
 // strcat(histBuffer,"m3");
  char *fmtValue = hydBuffer;
  tmSprite.setTextColor(c1,c2);
  tmSprite.drawString(fmtValue,timerWidth*0.5, timerHeight*0.5);
  tmSprite.unloadFont();
  tmSprite.loadFont(medium30);
  tmSprite.setTextColor(GREY,c2);
  tmSprite.drawString("(0-999)",timerWidth*0.5, timerHeight*0.8);
  tmSprite.unloadFont();
