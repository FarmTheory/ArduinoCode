...
SoftwareSerial BTSerial(7, 14); // RX | TX
...
int sy=116;
bool onOff=0;
bool areaMetric = true;
bool speedMetric = true;
bool endDelay = true;
//VARIABLES2
float ww;// = 10;
float tempWW;// = ww;
...
int hydTrigger;// = 300;
int tempHydTrigger;
int epromAddress[] = {0,16,32,48,64,70,76,82,88};
//ADDRESSES = PREV LOAD TIME, ww, TargetRate, trkSize, HydTrigger, AreaMetric, SpeedMetric, EndDelay
//Timing for Loads
int prevLoadTime = 120;
float loadTime = 0;
uint32_t unpauseTime;
uint32_t pauseTime;
int resetTimeOut = 120; //TIMER RESET
float targetSpeed = 0;
//PINS
//LED variables
int buzzerPin = 26;
...
int targetY = flowY+flowHeight+10;
int targetBX = 4;
int targetBY = 4;
bool targetOn = false;
//COUNTER
...
int counterY = targetY+targetHeight+10;
int counterBX = 14;
int counterBY = 4;
...
static uint32_t finishWaitTime = 1*60*1000; //4*60&1000
...
void loadEEPROM(){
  ...
  if (eepromWrite == 1 || resetBtnState == 1){
      ...
      writeIntIntoEEPROM(epromAddress[6], 0); //areaMetric  
      writeIntIntoEEPROM(epromAddress[7], 1); //speedMetric  
      writeIntIntoEEPROM(epromAddress[8], 1); //endDelay  
       //HISTORY
      for (int i=0; i<10; i++){
   ...
   int areaInt = readIntFromEEPROM(epromAddress[6]);
   int speedInt = readIntFromEEPROM(epromAddress[7]);
   int endInt = readIntFromEEPROM(epromAddress[8]);
   if (areaInt == 0){
      areaMetric = false;
    }
   if (speedInt == 0){
...
//PINS
void initPins(){
    int inPins[] = {hydPin, flowPin, resetPin};
    int outPins[] = {buzzerPin, r1Pin, r2Pin, r3Pin};
    for (int i=0; i<4; i++){
